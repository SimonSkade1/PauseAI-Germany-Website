      ...
      <section className="mx-auto max-w-5xl px-6 pb-12 md:px-10">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {filteredGroups.map((group) => (
            <div
              key={group.id}
              className="flex flex-col border-2 border-[#1a1a1a] bg-white transition-shadow hover:shadow-lg"
            >
              <div className="flex flex-1 flex-col p-5">
                {/* Header: city + messenger icons */}
                <div className="flex items-start justify-between gap-3">
                  <h3 className="font-section text-xl text-pause-black leading-tight">
                    {group.city}
                  </h3>
                  <div className="flex shrink-0 items-center gap-1.5 pt-0.5">
                    {group.links.whatsapp && (
                      <a
                        ...
                        title="WhatsApp"
                        className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-[#25D366] text-white transition-opacity hover:opacity-80"
                      >
                    ...
                    {group.links.signal && (
                      <a
                        ...
                        rel="noopener noreferrer"
                        title="Signal"
                        className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-[#3A76F0] text-white transition-opacity hover:opacity-80"
                      >
                    ...
                    {group.links.telegram && (
                      <a
                        ...
                        rel="noopener noreferrer"
                        title="Telegram"
                        className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-[#0088cc] text-white transition-opacity hover:opacity-80"
                      >
                    ...
                    {group.website && (
                      <a
                        ...
                        rel="noopener noreferrer"
                        title="Webseite"
                        className="inline-flex h-7 w-7 items-center justify-center rounded-full border border-[#1a1a1a]/30 bg-white text-pause-black transition-opacity hover:opacity-70"
                      >
                ...
                {/* Description */}
                {group.description && (
                  <p className="mt-2 font-body text-sm leading-relaxed text-pause-black/65">
                    {group.description}
                  </p>
                )}
                ...
                <div className="mt-4 border-t border-[#1a1a1a]/10 pt-3">
                  <p className="font-body text-xs text-pause-black/50">
                    Kontakt: {group.contact.name}{" · "}
                    <a href={`mailto:${group.contact.email}`} className="text-blue-600 underline hover:text-blue-800">
                      {group.contact.email}
                    </a>
                  </p>
                  <div className="mt-2 flex items-center gap-3">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 font-body text-xs text-pause-black/40 transition-colors hover:text-pause-black"
                    >
